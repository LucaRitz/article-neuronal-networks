...
die Korrektur der Gewichte im aktuellen Berechnungsschritt für den aktuellen Layer.
\\

Zuerst betrachtet man den Fall, wo es sich um den Output-Layer handelt. Um dies zu veranschaulichen, kann
die Abbildung \ref{fig:08_backpropagation_output} betrachtet werden. In der blauen Ebene der Abbildung sind die Werte ersichtlich,
...
die ebenso wie die Gewichte nummeriert sind. In dem Fall würde die Korrektur für $w_{ij}^r$ lauten\footnote{An dieser
Stelle aufgepasst: Es gibt noch einen Input mehr: $-1$, der Bias. Man könnte diesen ebenso als Input aus der vorherigen
Schicht betrachten.}:
\begin{align}
    \delta_i^r = \frac{\delta P_{err}}{\delta net_i^r}\\
    w_{ij, neu}^r = w_{ij, alt}^r + \lambda \cdot \delta_i^r \cdot \frac{net_i^r}{\delta w_{ij}^r}\\
    w_{ij, neu}^r = w_{ij, alt}^r + \lambda \cdot \delta_i^r \cdot z_{s}^{p}
\end{align}
...
\begin{algorithm}[H]
    \For{o = last downto 2}{
        ...
        \For{(t, h) = (1,1) to (i, j)}{
            \textcolor{BackpropOrange}{$\delta w_{th}^o$} $= \delta_t^o \cdot z_{th}^{o - 1}$\\
