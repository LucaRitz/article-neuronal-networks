...
bilden nun die zwei Fälle des Algorithmus.
\\
...
handelt und das einzig Bekannte ist somit der Fehlerwert $P_{err}$. Hierbei stehen nun die Angaben $r$ für die Schicht,
...
Schicht, $l$ für die Anzahl der Neuronen der Schicht $k$, $m$ für die Anzahl der Gewichte der Schicht $k$ (bei einem
Full-Connected Netzwerk gilt $m = i + 1$). Auf der rechten Seite können ebenfalls die bekannten Werte abgelesen werden.
Diese setzen sich nun aus den bereits vorhin gesehenen Termen $\delta_l^k$\footnote{Anzumerken sei hier, dass es sich bei $l$ nur
um einen Platzhalter handelt. Die Nummerierung geht von $1$ bis $l$, es existieren also $l$ $\delta$.} zusammen. Es sind die Ableitungen nach $net_l^k$.
Wie bereits erwähnt gibt es so viele Terme, wie Neuronen der Schicht $k$.
\begin{figure}[h!]
    \begin{center}
...
Man nehme nun an, dass es bei den Gewichten der Schicht $r$ die Inputs $x_{ij}^{r-1}$ der vorherigen Schicht $r-1$ gibt,
...
\begin{algorithm}[H]
    \For{o = last downto 2}{
        ...
        \For{(t, h) = (1,1) to (i, j)}{
            $\delta w_{th}^o = \delta_t^o \cdot w_{th}^{o}$\\
